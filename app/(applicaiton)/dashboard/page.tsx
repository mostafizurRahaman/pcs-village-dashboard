...
const statCards = [
  { icon: Users,        bg: "bg-chart-1",    label: "Total Users",    value: "12,543" },
  { icon: UserCheck,    bg: "bg-success",    label: "Active Users",   value: "8,342"  },
  { icon: FileText,     bg: "bg-chart-3",    label: "Total Posts",    value: "3,456"  },
  { icon: Users,        bg: "bg-chart-4",    label: "Total Groups",   value: "234"    },
  { icon: MessageSquare,bg: "bg-chart-5",    label: "Messages Sent",  value: "45,678" },
  { icon: UserPlus,     bg: "bg-destructive",label: "Total Referrals",value: "1,234"  },
]
    ...
    <ContentLayout title="Dashboard">
      <div className="flex flex-col gap-8 w-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {statCards.map(({ icon: Icon, bg, label, value }) => (
            <Card key={label} className="shadow-sm border-border">
              <CardContent className="p-6 flex flex-col gap-8">
                ...
                <div className="flex flex-col gap-1">
                  <Typography variant="Bold_H3" className="text-foreground !text-[24px]">{value}</Typography>
                  <Typography variant="Regular_H5" className="text-muted-foreground !text-[14px]">{label}</Typography>
                </div>
              </CardContent>
            </Card>
          ))}
            ...
            <CardContent className="p-6 pt-0 h-[320px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={dailyActiveData} margin={{ top: 20, right: 10, left: -20, bottom: 0 }}>
                  ...
                  <Bar dataKey="users" fill="var(--color-success)" radius={[0, 0, 0, 0]} maxBarSize={50} />
                </BarChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
          ...
          <CardContent className="p-0">
            <div className="flex flex-col">
              {recentActivities.map((activity, index) => (
                ...
                >
                  <div className="w-10 h-10 rounded-full bg-success shrink-0 flex items-center justify-center">
                    <Typography variant="Regular_P" className="text-success-foreground">
                      {activity.initial}
                    </Typography>
                  </div>
                  <div className="flex flex-col flex-1 pl-1">
                    <div className="flex items-center gap-1">
                      <Typography variant="SemiBold_H5" className="text-foreground !text-[14px]">
                        {activity.user}
                      </Typography>
                      <Typography variant="Regular_H5" className="text-foreground !text-[14px]">
                        {activity.action}
                      </Typography>
                    </div>
                    <Typography variant="Regular_H7" className="text-muted-foreground !text-[12px] mt-0.5">
                      {activity.time}
                    </Typography>
                  </div>
                </div>
