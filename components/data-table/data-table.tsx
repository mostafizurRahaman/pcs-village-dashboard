...
export function DataTable<TData extends ExportableData, TValue>({
  ...
  const getSelectedItems = useCallback(async () => {
    ...
    const selectedIdsArray = Object.keys(selectedItemIds).map(id => {
      ...
      return typeof id === 'string' ? Number.parseInt(id, 10) : id as number
    }
    );
    ...
    console.log('  Fetch by ids fn', fetchByIdsFn);

    try {
      // Fetch missing items in a single batch
      const fetchedItems = await fetchByIdsFn(idsToFetch as string[]);
  ...
  return (
    <div className="space-y-4">
      ...
      <div
        ...
        className="overflow-y-auto rounded-[8px] bg-white border border-[rgba(0,0,0,0.1)] table-container overflow-hidden"
      ...
      >
        <Table className={tableConfig.enableColumnResizing ? "resizable-table" : ""}>
          <TableHeader className="bg-[#e8e8dc] h-[56px]">
            {table.getHeaderGroups().map((headerGroup) => (
              ...
              >
                {headerGroup.headers.map((header) => (
                  <TableHead
                    className="px-6 py-2 relative text-left group/th text-[12px] font-medium text-[#1f3a5f] uppercase tracking-[0.6px]"
                    key={header.id}
            ...
            {isLoading ? (
              ...
              table.getRowModel().rows.map((row, rowIndex) => (
                <TableRow
                  ...
                  className="h-[73px] border-[rgba(0,0,0,0.1)] transition-colors hover:bg-slate-50"
                  onClick={(event) => {
                ...
                >
                  {row.getVisibleCells().map((cell, cellIndex) => (
                    <TableCell
                      className="px-6 py-2 truncate max-w-0 text-left"
                      key={cell.id}
